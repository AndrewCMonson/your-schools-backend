import { SchoolModel, UserModel, SessionModel, ReviewModel, } from "../models/index";
    ...
    Mutation: {
        addUser: async (_, { username, email, password }, { res }) => {
            if (!username || !email || !password) {
                throw new AuthenticationError("You need to provide a username, email, and password");
            }
        ...
        updateUserInfo: async (_, { username, email, zipcode, theme }, { user }) => {
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            const updatedUser = await UserModel.findByIdAndUpdate({ _id: user.id }, { username, email, zipcode, theme }, { new: true });
            if (!updatedUser) {
        ...
        updateUserPassword: async (_, { password, newPassword }, { user }) => {
            ...
            if (password === "" || newPassword === "") {
                throw new AuthenticationError("You need to provide a password and a new password");
            }
            if (password === newPassword) {
            ...
            const updatedUser = await UserModel.findByIdAndUpdate({ _id: user.id }, { password: hashedPassword }, { new: true });
            if (!updatedUser) {
                throw new AuthenticationError("Couldn't find user with this id");
            }
            return updatedUser;
        },
        adminUpdateUserInfo: async (_, { id, username, email, zipcode, theme, isAdmin }, { user }) => {
            if (!id)
                throw new Error("Please provide a user ID");
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            if (!user.isAdmin)
                throw new AuthenticationError("You need to be an admin to update a user");
            const updatedUser = await UserModel.findByIdAndUpdate({ _id: id }, { username, email, zipcode, theme, isAdmin }, { new: true });
            if (!updatedUser) {
        ...
        adminAddUser: async (_, { username, email, isAdmin }, { user }) => {
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            if (!user.isAdmin)
                throw new Error("You need to be an admin to add a user via the admin dashboard");
            if (!username || !email) {
        ...
        deleteUser: async (_, { id }, { user }) => {
            ...
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            if (!user.isAdmin)
                throw new AuthenticationError("You need to be an admin to delete a user");
        ...
        addToFavorites: async (_, { schoolId }, { user }) => {
            if (!schoolId)
                throw new Error("Please provide a school ID");
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            const updatedUser = await UserModel.findOneAndUpdate({ _id: user.id }, { $addToSet: { favoriteIds: schoolId } }, { new: true });
            if (!updatedUser) {
                throw new AuthenticationError("Couldn't find user with this id");
            }
            return updatedUser;
        ...
        removeFromFavorites: async (_, { schoolId }, { user }) => {
            if (user) {
                const updatedUser = await UserModel.findByIdAndUpdate({ _id: user.id }, { $pull: { favoriteIds: schoolId } }, { new: true });
                if (!updatedUser) {
                    throw new AuthenticationError("Couldn't find user with this id");
                }
        ...
        recoverPassword: async (_, { email }) => {
            ...
            const hashedPassword = await hashPassword(tempPassword);
            await UserModel.findByIdAndUpdate({ _id: user.id }, { password: hashedPassword }, { new: true });
            await sendRecoveryEmail(email, tempPassword);
        ...
        addReview: async (_, { rating, review, schoolId }, { user }) => {
            ...
            const hasReviewed = userReviews.some((userReview) => userReview.school.toString() === schoolId);
            if (hasReviewed) {
            ...
            await SchoolModel.findByIdAndUpdate({ _id: schoolId }, { $addToSet: { reviews: newReview.id } }, { new: true });
        ...
        updateSchoolInfo: async (_, { id, name, address, city, state, zipcode, phone, website, email, description, rating, offers_daycare, age_range, early_enrollment, min_tuition, max_tuition, days_open, days_closed, opening_hours, closing_hours, min_enrollment, max_enrollment, min_student_teacher_ratio, max_student_teacher_ratio, avatar, isVerified, }, { user }) => {
            if (!id)
                throw new Error("Please provide a school ID");
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            const updatedSchool = await SchoolModel.findByIdAndUpdate({ _id: id }, {
                name,
                address,
                city,
                state,
        ...
        addSchool: async (_, { name, address, city, state, zipcode }, { user }) => {
            if (!name || !address || !city || !state || !zipcode) {
                throw new Error("You need to provide a name, address, city, state, and zipcode");
            }
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            if (!user.isAdmin)
                throw new AuthenticationError("You need to be an admin to add a school");
        ...
        deleteSchool: async (_, { id }, { user }) => {
            ...
            if (!user)
                throw new AuthenticationError("You need to be logged in");
            if (!user.isAdmin)
                throw new AuthenticationError("You need to be an admin to delete a school");
        ...
        logout: async (_, __, { user, res, req }) => {
            if (user) {
                try {
